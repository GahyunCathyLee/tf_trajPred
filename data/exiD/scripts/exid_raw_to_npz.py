...
def make_windows_for_tracks_csv(
    ...
    nb_ids_cols = []
    multi_nb: Dict[Tuple[int, int], np.ndarray] = {}  # (row_index, slot_k) -> candidate ids

    def extract_ids_semicolon(cell: object) -> np.ndarray:
        s = str(cell).strip()
        ...
        ids = []
        for t in toks:
            # 혹시 공백/기타가 섞이면 숫자만 남기기 (안전)
            m = re.fullmatch(r"\d+", t)
            if m:
                ids.append(int(t))
            else:
                # 숫자만 추출 가능한 경우에만 허용 (예: "12 " 같은)
                mm = re.findall(r"\d+", t)
                if len(mm) == 1:
                    ids.append(int(mm[0]))
    ...
    for k, col in enumerate(NEIGHBOR_COLS_8):
        if col in df.columns:
            raw = df[col].to_numpy()
            col_ids = np.full(len(df), -1, dtype=np.int32)

            for i in range(len(df)):
                cands = extract_ids_semicolon(raw[i])
                ...
                if cands.size == 0:
                    ...
                    # 기본값은 첫 번째로 두되, "복수 후보"는 따로 저장해두고 나중에 closest로 교체
                    col_ids[i] = cands[0]
                    multi_nb[(i, k)] = cands

            nb_ids_cols.append(col_ids)
        else:
        ...
        for i in range(0, n - win_len + 1, stride):
            ...
            for t in range(T):
                ...
                for k in range(K):
                    ...
                    key = (int(ego_r), int(k))
                    if key in multi_nb:
                        ego_x = float(x_shift[ego_r])
                        ego_y = float(y_shift[ego_r])

                        best_nid = -1
                        best_d2 = 1e30

                        for cand in multi_nb[key]:
                            cand = int(cand)
                            d_pos_c = frame_to_pos.get(cand)
                            if d_pos_c is None:
                                continue
                            pos_in_c = d_pos_c.get(fr)
                            if pos_in_c is None:
                                continue

                            cand_row = veh_rows[cand][pos_in_c]
                            dx = float(x_shift[cand_row] - ego_x)
                            dy = float(y_shift[cand_row] - ego_y)
                            d2 = dx * dx + dy * dy
                        ...
                        if best_nid < 0:
                            continue  # 후보들이 이 frame에 없으면 skip
                        nid = best_nid
                    ...
                    # 2. dx_time 계산 및 gate 결정
                    dx_time = dx / (dvx + (eps_gate if dvx >= 0 else -eps_gate))
...
def main():
    ...
    ap.add_argument("--t_back", type=float, default=5, help="Time gate back")
    ap.add_argument("--vy_eps", type=float, default=0.27, help="Lateral velocity epsilon for LC state")
