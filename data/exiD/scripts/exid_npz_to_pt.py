...
- ego_safety: (N,T,5) float32   [leadDHW, leadDV, leadTHW, leadTTC, lead_exists]
- y_fut: (N,Tf,2) float32
- nb_hist: (N,T,K,9) float32
- nb_mask: (N,T,K) bool
- ego_static: (N,Ds) float32
- nb_static: (N,K,Ds) or (N,T,K,Ds) float32
...
def _maybe_concat_static(out: dict) -> None:
    """In-place concat of static features into x_hist / nb_hist if present."""
...
def _maybe_make_x_last_abs(out: dict) -> None:
    """In-place add x_last_abs=(N,2) from x_hist last timestep's first two dims."""
...
def convert_one(
    ...
    try:
        ...
        if check_shapes:
            if "x_hist" in out:
                x_hist = out["x_hist"]
                assert isinstance(x_hist, torch.Tensor) and x_hist.ndim == 3, f"x_hist must be (N,T,D), got {tuple(x_hist.shape)}"
